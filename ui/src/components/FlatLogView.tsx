// Flat, filterable, virtualized log-stream view — CmdBar (filters), pinned
// strip, virtualized log stream + minimap, status bar. URL is the single
// source of truth for filters/ts/pin/sel (see hooks/useUrlState.ts,
// lib/filters.ts) — everything here derives from `params` and writes back
// through `setParams`, so a copied URL reproduces the exact view in a fresh
// tab. Extracted from SessionDetailPage so a plugin detail view can compose
// it (see FlatLogViewProps.baseFilters) — the crumb bar header stays in
// SessionDetailPage since it needs the session summary poll, which this
// component receives as a prop instead of owning.
import { useCallback, useEffect, useMemo, useRef, useState } from 'react'
import { useNavigate } from 'react-router-dom'
import { useVirtualizer } from '@tanstack/react-virtual'
import { purgeEvents, type SessionSummary, type StoredEvent } from '../api'
...
import { formatTs, parseTsMode, type TsMode } from '../lib/time'
...
import { Minimap, type MinimapErrorMark } from './Minimap'
...
export interface FlatLogViewProps {
  ...
  // Optional escape hatch for the parent's crumb bar, which renders the
  // "tail live"/"tail paused" chip: tail state (live vs. paused,
  // pending-while-paused count) and the resume action both live in this
  // component's useEventStream call, not in the parent — SessionDetailPage
  // has no other way to reflect them in its header. Reported via effect
  // rather than lifted state so FlatLogView keeps owning tail state; the
  // parent only mirrors it for display. Not part of the brief's literal
  // prop list (sessionId/session/baseFilters) — added because omitting it
  // would silently drop the crumb-bar tail indicator, a visible behavior
  // change from pre-refactor SessionDetailPage.
  onTailChange?: (tail: 'live' | 'paused', pendingCount: number, resume: () => void) => void
}

export function FlatLogView({ sessionId, session, baseFilters, onTailChange }: FlatLogViewProps) {
  const id = sessionId
  ...
  // mapping, so both can be unit-tested without mounting this component.
  //
  ...
  // Mirror tail state up to the parent's crumb-bar chip — see onTailChange
  // doc above. `resumeAndScrollBottom` and (if the caller memoized it)
  // `onTailChange` are stable across renders, so this only actually fires
  // when state.tail/state.pendingCount change, not on every render.
  useEffect(() => {
    onTailChange?.(state.tail, state.pendingCount, resumeAndScrollBottom)
  }, [state.tail, state.pendingCount, resumeAndScrollBottom, onTailChange])
